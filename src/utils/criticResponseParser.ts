...
/** Maximum line length processed by response parser regexes to prevent ReDoS */
export const MAX_PARSER_LINE_LENGTH = 4096
...
export function parseConcernsFromResponse(response: string): Concern[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]
    if (line.length > MAX_PARSER_LINE_LENGTH) continue
